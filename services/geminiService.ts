...
2. Знайти ключові дані: Номер ВМД (митната декларація), Опис товарів, Коди УКТЗЕД, Кількість.
3. ЗНАЙТИ ЯКЩО Є (не обов'язково):
    - Номер проформи-рахунку (інвойсу) та його дату.
    - Номер додатку (угоди/контракту) та його дату.
    Багато фірм не використовують додатки або не вказують проформи у висновках, тому якщо їх немає в документах — це НОРМАЛЬНО. Не вигадуйте їх.
4. Згенерувати "Експертний висновок", заповнивши наданий користувачем HTML-ЗРАЗОК.
...
- Якщо даних немає (наприклад, немає номеру додатку), просто видаліть відповідний плейсхолдер або залиште місце пустим, щоб текст виглядав логічно.
...
Знайди та випиши окремо наступні дані:
- Номер ВМД (обов'язково)
- Основні товари та їх коди УКТЗЕД (обов'язково)
- Загальна кількість (обов'язково)
- Номер проформи/інвойсу (ТІЛЬКИ ЯКЩО Є В ЯВНОМУ ВИГЛЯДІ)
- Дата проформи/інвойсу (ТІЛЬКИ ЯКЩО Є В ЯВНОМУ ВИГЛЯДІ)
- Номер додатку (ТІЛЬКИ ЯКЩО Є В ЯВНОМУ ВИГЛЯДІ)
- Дата додатку (ТІЛЬКИ ЯКЩО Є В ЯВНОМУ ВИГЛЯДІ)
...
6. Якщо в шаблоні є згадка про Додаток або Проформу, але в документах їх немає - прибери згадку про їх номери/дати з тексту шаблону, щоб він виглядав грамотно.
...
`;
  ...
  try {
    ...
    // Clean up response if it contains markdown fences despite requested JSON mimeType
    ...
    try {
        ...
        return {
            fullText: `<div style="color: red; padding: 20px; border: 1px solid red;">
                        <h3>Помилка обробки відповіді</h3>
                        <p>Не вдалося розпізнати формат відповіді від ШІ.</p>
                        <p>Спробуйте ще раз або перевірте документи.</p>
                        <details>
                            <summary>Технічні деталі (сирий текст відповіді)</summary>
                            <pre style="white-space: pre-wrap; font-size: 10px; margin-top: 10px;">${responseText.replace(/</g, '&lt;')}</pre>
                        </details>
                       </div>`,
            extractedData: {}
        };
